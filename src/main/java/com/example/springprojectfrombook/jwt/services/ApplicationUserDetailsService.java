package com.example.springprojectfrombook.jwt.services;

import com.example.springprojectfrombook.jwt.models.UserPrincipal;
import com.example.springprojectfrombook.user.entity.UserEntity;
import com.example.springprojectfrombook.user.service.UserService;
...
public class ApplicationUserDetailsService implements UserDetailsService {
    ...
    private Boolean verifyPasswordHash(String password,
        ...
        if(password.isBlank() || password.isEmpty()) throw new IllegalArgumentException(
                "Password cannot be emoty or whitespace only string"
        );

        if(storedHash.length != 64) throw new IllegalArgumentException(
                "Invalid length of password hash (63 bytes expected)"
        );
    ...
    public UserEntity authenticate(String email, String password) throws NoSuchAlgorithmException{
        ...
        var verified  = verifyPasswordHash(
                password, userEntity.getStoredHash(), userEntity.getStoredSalt()
        );
